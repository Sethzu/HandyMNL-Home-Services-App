...
import 'package:flutter_rating_bar/flutter_rating_bar.dart'; // For the rating modal
...
class _WorkerBookingsPageState extends State<WorkerBookingsPage> {
          ...
          Expanded(
            child: StreamBuilder(
              stream: FirebaseFirestore.instance
                  ...
                  .orderBy('timestamp',
                      descending: true) // Show most recent offers first
                  .snapshots(),
                        ...
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) => WorkerChecksCustomerProfile(
                              customerId:
                                  offer['customerId'], // Pass customerId
                            ),
                          ),
                        );
                      },
                        ...
                        child: ListTile(
                          ...
                          subtitle: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 8),
                              _buildActionButton(context, offer),
                            ],
                          ),
  ...
  Widget _buildActionButton(BuildContext context, QueryDocumentSnapshot offer) {
    if (_selectedSegment == 'Pending') {
      // Message button for Pending section
      return Padding(
        padding: const EdgeInsets.symmetric(
            vertical: 3), // Padding around the button
        child: Align(
          alignment:
              Alignment.centerLeft, // Align the button slightly to the left
          child: SizedBox(
            width: MediaQuery.of(context).size.width *
                0.4, // 50% of the screen width
            height: 35, // Adjust height if necessary
            child: ElevatedButton(
              onPressed: () => _handleMessageButtonPressed(context, offer),
              style: ElevatedButton.styleFrom(
                backgroundColor: Colors.white,
                side: const BorderSide(
                    color: Colors.blueAccent,
                    width: 3), // Set border width to 3
              ),
              child: const Text(
                'Message',
      ...
      return Padding(
        ...
        child: Align(
          alignment:
              Alignment.centerRight, // Align the button slightly to the right
          child: SizedBox(
            width: MediaQuery.of(context).size.width *
  ...
  Future<void> _handleMessageButtonPressed(
      BuildContext context, QueryDocumentSnapshot offer) async {
    ...
    final conversationDoc = await FirebaseFirestore.instance
        ...
        .where('customerId', isEqualTo: offer['customerId'])
        .limit(1)
        .get();
    ...
    // Navigate to ChatPage with the conversationId and customer details
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ChatPage(
          conversationId: conversationId,
          receiverFirstName: offer['customerName'],
          receiverLastName: '',
          receiverEmail: offer['customerEmail'],
          receiverUid: offer['customerId'],
        ),
      ),
    );
  }
  ...
  Widget _buildTrailingWidget(
    ...
    if (_selectedSegment == 'Pending') {
      ...
      // No pop-up menu; we removed the pop-up and message button
      return const SizedBox.shrink();
    } else {
      return const Text(
        'Declined',
  ...
  void _showWorkerRateReviewModalBottomSheet(
      BuildContext context, QueryDocumentSnapshot offer) {
...
class WorkerRateReviewBottomSheet extends StatefulWidget {
  ...
  @override
  _WorkerRateReviewBottomSheetState createState() =>
      _WorkerRateReviewBottomSheetState();
}

class _WorkerRateReviewBottomSheetState
    extends State<WorkerRateReviewBottomSheet> {
  ...
  Future<void> _submitReview() async {
    ...
    await _updateCustomerAverageRating(
        widget.offer['customerId'], widget.offer['customerName']);
  ...
  Future<void> _updateCustomerAverageRating(
